...
class BatailleNavaleGraphique extends BatailleNavale {
	...
	private BoutonCacher boutonCacher = new BoutonCacher(panelPlateau);
	...
	public void actualiserInfo(int nbBateauActif,int nbBateauAutre) {
		panelInfo.setText("C'est au joueur " + joueurActif + " de jouer");
	...
	public void joueurSuivant() {
		int nbBateauActif,nbBateauAutre; // pour ne pas faire plus de if
		boutonCacher.doClick();
		if (joueurActif == 'a') {
			joueurActif = 'b';
			nbBateauActif = nbBateauxB;
			nbBateauAutre = nbBateauxA;
		}
		else {
			joueurActif = 'a';
			nbBateauActif = nbBateauxA;
			nbBateauAutre = nbBateauxB;
		}
		if (nbBateauAutre == 0)
			terminerPartie();
		...
		System.out.println("----------------------------------------------------");
		actualiserPlateau();
		actualiserInfo(nbBateauActif, nbBateauAutre);
		afficherPlateau();
	}
